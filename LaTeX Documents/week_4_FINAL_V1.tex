...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}
        [node distance=2cm,
        source/.style={draw, circle, thick, label=below:{MV Supply}},
        ct/.style={draw, thick, rectangle, minimum size=1cm, label=above:{TC}},
        load/.style={draw, thick, minimum height=1cm, minimum width=0.5cm},
        line/.style={thick},
        arrow/.style={-latex, thick}
    ]
        % MT Ring
        \node[source] (mt) {20kV};
        \node[ct, above left=2cm and 3cm of mt] (ct1) {TC-1};
        \node[ct, above right=2cm and 3cm of mt] (ct2) {TC-2};
        \draw[line, red] (mt.120) .. controls +(120:3cm) and +(180:3cm) .. (ct1.west);
        \draw[line, red] (mt.60) .. controls +(60:3cm) and +(0:3cm) .. (ct2.east);
        \draw[line, red] (ct1.north) .. controls +(up:1.5cm) and +(up:1.5cm) .. (ct2.north);

        % Loads from CT-1
        \node[load, below=2cm of ct1, label=below:{R\&D Facility}] (ind1) {};
        \node[load, right=1cm of ind1, label=below:{Offices}] (off) {};
        \draw[arrow] (ct1) -- node[midway, right]{LGA-1} (ind1);
        \draw[arrow] (ct1) -- node[midway, right]{LGA-2} (off);

        % Loads from CT-2
        \node[load, below=2cm of ct2, label=below:{Museum}] (mus) {};
        \node[load, right=1cm of mus, label=below:{Facilities 2\&3}] (ind23) {};
        \node[load, left=1cm of mus, label=below:{Residential}] (res) {};
        
        \draw[arrow] (ct2) -- node[midway, right]{LGA-3} (mus);
        \draw[arrow] (ct2) -- node[midway, left]{LGA-4} (ind23);
        \draw[arrow] (ct2) -- node[midway, left]{LGA-5} (res);
        
        % Alumbrado (shared)
        \node[load, below=5cm of mt, label=below:{Lighting}] (alum) {};
        \draw[arrow] ($(ct1)!.5!(ct2)$) |- (alum);
    \end{tikzpicture}
    }
...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[thick, scale=0.8, every node/.style={scale=0.8}]
        \draw (-1,0) -- (7,0);
        \node[above] at (3,0) {General Command and Protection Panel (GCPP)};
        % LGA
        \draw[->] (0,2) -- (0,0.2); \node[above] at (0,2) {LGA Offices (40A)};
        % Sub-circuits
...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[thick, scale=0.8, every node/.style={scale=0.8}]
        \draw (-1,0) -- (9,0);
        \node[above] at (4,0) {GCPP R\&D Industrial Facility};
        % LGA
        \draw[->] (0,2) -- (0,0.2); \node[above] at (0,2) {LGA R\&D Facility (500A)};
        % Sub-circuits
        \draw (1,-0.2) -- (1,-2); \draw[->] (1,-2) -- (1,-3); \node[below] at (1,-3){C1: Lighting};
        \draw (3,-0.2) -- (3,-2); \draw[->] (3,-2) -- (3,-3); \node[below] at (3,-3){C2: Power};
        \draw (5,-0.2) -- (5,-2); \draw[->] (5,-2) -- (5,-3); \node[below] at (5,-3){C3: HVAC};
        \draw (7,-0.2) -- (7,-2); \draw[->] (7,-2) -- (7,-3); \node[below] at (7,-3){C4: R\&D Equipment};
    \end{tikzpicture}
...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[thick, scale=0.8, every node/.style={scale=0.8}]
        \draw (-1,0) -- (9,0);
        \node[above] at (4,0) {GCPP Technology Museum};
        % LGA
        \draw[->] (0,2) -- (0,0.2); \node[above] at (0,2) {LGA Museum (200A)};
        % Sub-circuits
