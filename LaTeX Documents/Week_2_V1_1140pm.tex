...
Where $T_s$ is the cable surface temperature and $T_{\text{soil}}$ is the ambient soil temperature. The thermal resistance of the soil, $R_{\text{th,soil}}$, is calculated using an approximation derived from Kennelly's formula:

\begin{equation}
R_{\text{th,soil}} = \frac{\rho_{\text{soil}}}{2 \pi L} \ln\left( \frac{4h}{D} \right)
\end{equation}
...
\begin{itemize}
    ...
    \item $D$: Outer diameter of the cable (m)
\end{itemize}

\subsubsection{Overhead Installation}
For an overhead line exposed to the atmosphere, the heat generated internally by the conductor ($P_{gen} = I^2 R$) must be dissipated to the surroundings to reach thermal equilibrium. The primary mechanisms for heat dissipation are convection and radiation, while the cable also absorbs heat from solar radiation. The heat balance is given by:
\begin{equation}
...
\begin{itemize}
    ...
    \item \textbf{Nusselt Number ($Nu$) Correlation (Example for Horizontal Cylinder):} A common correlation covering a wide range of $Ra$ is:
        \begin{equation}
...
\begin{itemize}
    ...
    \item \textbf{Nusselt Number ($Nu$) Correlation (Example for Cylinder in Crossflow):} A widely used correlation (Churchill \& Bernstein) valid for a broad range of $Re$ and $Pr \ge 0.2$ is: % Corrected & to \&
        \begin{equation}
        Nu_{\text{forced}} = 0.3 + \frac{0.62 Re^{1/2} Pr^{1/3}}{\left[1 + (0.4/Pr)^{2/3}\right]^{1/4}} \left[1 + \left(\frac{Re}{282000}\right)^{5/8}\right]^{4/5}
        \end{equation}
        Simpler correlations like $Nu = C Re^m Pr^{1/3}$ are often used for specific $Re$ ranges.
\end{itemize}

\subparagraph{c) Combined Convection:}
In many real-world scenarios, both natural and forced convection effects might be present (e.g., low wind speeds). More complex correlations exist to combine $Nu_{\text{nat}}$ and $Nu_{\text{forced}}$, often involving vector summation or other empirical methods, to determine the overall effective $Nu$ and subsequently $h_c$.
...
For accuracy, the conductor's AC resistance ($R_{AC}$) is calculated, accounting for skin and proximity effects (\texttt{E3b\_AC\_Resistance\_Factor.m}). The maximum admissible current ($I_{\text{max}}$) is the current that causes the conductor to reach its maximum insulation temperature (e.g., \SI{90}{\celsius} for XLPE), as determined in \texttt{E2\_MaximumCurrent.m}.
\begin{equation}
...
\begin{figure}[h!]
    ...
    \begin{tikzpicture}
        \begin{axis}[
            ...
            width=\textwidth, height=8cm, grid=major, legend pos=north west,
        ]
        ...
        \addplot[dotted, color=green!50!black] coordinates {(218,20) (218,90)};
        \addlegendentry{Calculated $I_{\text{max}}$ (218 A)}
        \end{axis}
    \end{tikzpicture}
...
\begin{figure}[h!]
    ...
    \begin{tikzpicture}
        \begin{axis}[
            ...
            width=\textwidth, height=8cm, grid=major,
        ]
