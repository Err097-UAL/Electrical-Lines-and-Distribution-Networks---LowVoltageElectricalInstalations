...
% =========================================================================
% MODIFIED: The file CrossSection.m has been eliminated, being a primitive
% main function
% - Converted into a function to be called from a master script.
% - All user menus are now centered.
% =========================================================================

%% --- Scenario Selection ---
sizing_scenarioChoice = centeredMenu2('Select Sizing Criterion:', 'Voltage Drop (Student A)', 'Current-Carrying Capacity (Student B)', 'Economic Optimization (Student C)');

if sizing_scenarioChoice == 0
...
lineLength = input('Enter total line length [m]: ');
loadCurrent = input('Enter design load current [A]: ');
cos_phi = input('Enter power factor (e.g., 0.85): ');
materialChoice = centeredMenu2('Select Conductor Material:', 'Copper', 'Aluminum');
if materialChoice == 1, material = 'Copper'; sigma = sigma_copper;
else, material = 'Aluminum'; sigma = sigma_aluminum; end

%% --- Technical Sizing ---
% 1. Sizing by Voltage Drop
deltaU_max_percent = input('Enter max voltage drop [%] (e.g., 5 for REBT power): ');
deltaU_max_V = (deltaU_max_percent / 100) * U_source;
s_vd = G2_calculateRequiredSection(lineType, lineLength, loadCurrent, cos_phi, sigma, deltaU_max_V);
[s_standard_vd, ~, all_sections] = G3_selectStandardSection(s_vd, material);

% 2. Sizing by Ampacity
[~, ampacity_selected, ~] = G3_selectStandardSection(s_standard_vd, material);
s_amp = s_standard_vd;
while ampacity_selected < loadCurrent
    [s_amp, ampacity_selected, ~] = G3_selectStandardSection(s_amp + 0.1, material); % Go to next size
end

% 3. Determine Final Technical Section
s_technical = max(s_standard_vd, s_amp);
[~, ampacity_final, ~] = G3_selectStandardSection(s_technical, material);

fprintf('\\n--- TECHNICAL SIZING RESULTS ---\\n');
fprintf('Required section by V-Drop: %.2f mm^2 -> Standard: %.2f mm^2\\n', s_vd, s_standard_vd);
fprintf('Required section by Ampacity: %.2f mm^2 (Ampacity: %.1f A)\\n', s_amp, ampacity_final);
fprintf('--------------------------------------------------\\n');
fprintf('Final Technically Compliant Section: %.2f mm^2\\n', s_technical);
fprintf('--------------------------------------------------\\n');

%% --- Economic Optimization (if selected) ---
if sizing_scenarioChoice == 3
    ...
    for i = 1:num_sections
        [total_costs(i), cable_costs(i), loss_costs(i)] = G4_calculateLifecycleCost(...
    ...
    fprintf('Associated Lifecycle Cost: %.2f Euros\\n', min_cost);
    if s_optimal == s_technical
        ...
        fprintf('Recommendation: Using a larger cable (%.2f mm^2 vs %.2f mm^2) could save %.2f Euros over %d years.\\n', s_optimal, s_technical, cost_at_technical-min_cost, LCC_years);
    end
    
    % Plot the economic analysis
...
final_vd_percent = G6_verifyFinalDesign(lineType, lineLength, loadCurrent, cos_phi, sigma, s_final, U_source);
fprintf('\\n--- FINAL DESIGN VERIFICATION ---\\n');
fprintf('Chosen Section: %.2f mm^2 (%s)\\n', s_final, material);
fprintf('Actual Voltage Drop with this section: %.2f %%\\n', final_vd_percent);
if final_vd_percent <= deltaU_max_percent
    disp('VERIFICATION PASSED: Voltage drop is within limits.');
else
    disp('VERIFICATION FAILED: Voltage drop exceeds limits.');
end
...
%         s_vd = G2_calculateRequiredSection(lineType, lineLength, loadCurrent, cos_phi, sigma, deltaU_max);
%         fprintf('Theoretical section required for voltage drop: %.2f mm^2\n', s_vd);
%         [s_final, ampacity] = G3_selectStandardSection(s_vd, material);
...
%         end
%         final_vd = G6_verifyFinalDesign(lineType, lineLength, loadCurrent, cos_phi, sigma, s_final, U_source);
...
%         [s_final, ~, all_sections] = G3_selectStandardSection(0, material); % Get the table
% 
...
%         s_vd = G2_calculateRequiredSection(lineType, lineLength, loadCurrent, cos_phi, sigma, deltaU_max);
%         [s_vd_std, ~] = G3_selectStandardSection(s_vd, material);
% 
%         [~, ~, all_sections] = G3_selectStandardSection(0, material);
%         s_ampacity_options = all_sections([all_sections.ampacity] >= loadCurrent, :);
