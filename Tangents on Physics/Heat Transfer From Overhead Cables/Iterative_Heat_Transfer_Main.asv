...
% 2. Sets fixed material properties and simulation details.
% 3. Calculates derived parameters based on user inputs.
...
if strcmpi(scenario.location, 'overhead')
    ...
    rho_soil = input('Enter soil thermal resistivity (K-m/W) (e.g., 1.2): ');
...
t_end_hours = input('Enter simulation time (hours): ');
sim.t_end = t_end_hours * 3600; % (s)
params.L = input('Enter line length (m) (1.0 for per-unit-length analysis): ');

% --- Cable Geometry Inputs ---
...
% --- Material Property Inputs (Critical) ---
params.conductor.res_ref = input('Enter conductor electrical resistance at 20 C (Ohm/m): ');

disp('------------------------------------------');
disp('User inputs gathered. Using fixed material properties for other values.');

% -------------------------------------------------------------------------
% 2. FIXED MATERIAL & SIM PARAMETERS
% (Values not requested from user)
% -------------------------------------------------------------------------
...
% --- Material Properties ---
% Conductor (Copper properties, except res_ref)
